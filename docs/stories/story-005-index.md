...
7. [ ] The metadata store persists all `Function` fields including `summary` and `import_statement`
...
- [ ] 3. Implement metadata store in SQLite (or JSON file) mapping `faiss_id -> Function dict`
- [ ] 4. Implement `build(functions, embeddings, path)`
- [ ] 5. Implement `load(path) -> Index`
...
The metadata store must store `summary` and `import_statement` as they are precomputed at index time and served directly in query responses without any LLM call.
